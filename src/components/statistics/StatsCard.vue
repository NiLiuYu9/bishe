<!--
  =====================================================
  统计卡片组件 StatsCard —— 相当于后端的仪表盘统计卡片
  =====================================================
  
  【核心概念】展示单个统计指标（如总调用次数、总收入等）
  支持显示环比增长率和趋势图标
  
  【后端类比】相当于后端的仪表盘统计卡片，类似管理后台首页的数字展示
  
  【Vue 组件核心概念（给后端开发者）】
    - props（:title）—— 指标标题，相当于后端的方法参数
    - props（:value）—— 指标值
    - props（:prevValue）—— 上期值（用于计算环比）
    - props（:suffix）—— 值的后缀（如"次"、"元"）
    - computed —— 计算环比增长率，相当于后端的 getter 方法
-->
<template>
