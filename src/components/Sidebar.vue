<!--
  =====================================================
  侧边栏导航组件 Sidebar —— 相当于后端页面的侧边导航栏
  =====================================================
  
  【核心概念】根据用户角色（普通用户/管理员）显示不同的导航菜单
  展示 API 分类列表，点击分类可筛选 API 列表
  
  【后端类比】相当于后端页面的侧边导航栏，展示分类筛选项
  
  【Vue 组件核心概念（给后端开发者）】
    - props（:types）—— 父组件传入的API分类列表，相当于后端的方法参数
    - props（:activeTypeId）—— 当前选中的分类ID
    - emit（@type-click）—— 子组件触发的事件，点击分类时通知父组件，相当于后端的回调函数
    - emit（@page-click）—— 点击页面链接时通知父组件
-->
<template>
  <aside class="sidebar">
