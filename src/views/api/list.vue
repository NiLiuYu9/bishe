<!--
  =====================================================
  API市场列表页 —— 相当于后端的 API 列表页面
  =====================================================
  
  【核心概念】展示所有已上架的API，支持按分类/关键词/排序筛选
  
  【后端类比】API 列表页面，对应后端 ApiController.list()
    类似电商平台的商品列表，支持分页、搜索、筛选
  
  【页面生命周期（给后端开发者）】
    - onMounted → 加载API列表和分类数据
    - 调用 apiManagement.getApiList() → 相当于后端调用 ApiService.list()
    - 分页/筛选变化 → 重新调用API获取数据
-->
<template>
