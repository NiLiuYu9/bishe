<!--
  =====================================================
  首页 —— 相当于后端的首页 Controller 渲染的页面
  =====================================================
  
  【核心概念】展示平台介绍、热门API、最新需求等，为公开页面
  
  【后端类比】首页 Controller 渲染的页面，类似 portal 首页
  
  【页面生命周期（给后端开发者）】
    - onMounted → 相当于后端的 @PostConstruct，页面加载时获取热门API和最新需求数据
    - 调用 API 获取数据 → 相当于后端调用 Service 获取数据
    - 数据绑定到模板 → 相当于后端将数据传给视图渲染
-->
<template>
  <div class="home-page">
